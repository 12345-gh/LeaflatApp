...
var
  fmRectangleAdd: TfmRectangleAdd;

implementation

{$R *.dfm}

function TfmRectangleAdd.CheckValid: boolean;
var
  HintText: string;
begin
  HintText:='';
...
procedure TfmRectangleAdd.cxbtCancelClick(Sender: TObject);
begin
  ModalResult:=mrCancel;
end;

procedure TfmRectangleAdd.cxbtOKClick(Sender: TObject);
begin
  if (not CheckValid) then Exit;
  ModalResult:=mrOk;
end;

procedure TfmRectangleAdd.cxteNamePropertiesChange(Sender: TObject);
begin
  cxteName.Style.Color:=clWindow;
end;

procedure TfmRectangleAdd.cxteTypePropertiesChange(Sender: TObject);
begin
